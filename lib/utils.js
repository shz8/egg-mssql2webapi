...
module.exports = {
  ...
  async initapiTable(ctx) {
    if (!apitable) {
      ...
      if (ctx.app.config.mssql2webapi) {
        let urlprefix = ctx.app.config.mssql2webapi.urlprefix.tr || '';
        urlprefix = '/' + urlprefix.trim().trim('/').trim();
  ...
  async getapi(ctx) {
    for (let idx = 0; idx < apitable.length; idx++) {
      if (ctx.url.toLowerCase().indexOf(apitable[idx].path.toLowerCase().trim()) == 0) {
        return apitable[idx];
      }
    }
    return null;
